...
def check_permissions():
    if os.getuid() == 0:
        ...
        os.system("clear")
        print(
            "{0}[!]{1} ¡Permiso denegado! Recuerde ejecutar: {2}sudo {1}python3 KitHack.py".format(
                RED, DEFAULT, GREEN
            )
        )
        exit(0)
...
def main():
    ...
    if option == "01":
        ...
        print(
            "{0}05){1} PhisherMan   {2}Herramienta phishing con 17 plantillas.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
        ...
        print(
            "{0}07){1} Bluepot                 {2}Honeypot Bluetooth escrito en Java.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}08){1} Setoolkit               {2}Marco de prueba de penetración de código abierto diseñado para la ingeniería social.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}09){1} A2sv                    {2}Vulnerabilidad de escaneo automático a SSL.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}10){1} 4nonimizer              {2}Anonimiza la IP pública utilizada para navegar por Internet mediante proveedores VPN.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}11){1} Easysploit              {2}Automatización de Metasploit.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}12){1} NXcrypt                 {2}Inyección de malware en archivos con formato python.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}13){1} KnockMail               {2}Verifica si existe un correo electronico".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}14){1} RkHunter                {2}Herramienta de Unix que detecta los rootkits, puertas traseras y exploits locales.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}15){1} HeraKeylogger           {2}Chrome Keylogger Extension.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}16){1} ZLogger                 {2}Keylogger remoto persistente para Windows y Linux.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}17){1} Xerosploit              {2}Kit de herramientas de pruebas de penetración.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}18){1} Slacksec                {2}Kit basico de herramientas hacking.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}19){1} Katana-Framework        {2}Unifica distintas herramientas funcionales para pruebas de penetración.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}20){1} Z0172CK-Tools           {2}Hacking Tools Z0172CK.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}21){1} Cam-Hack                {2}Método avanzado para piratear la cámara de un móvil o una PC con un enlace.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}22){1} Onex                    {2}Biblioteca de herramientas para hackers.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}23){1} Ransom0                 {2}Ransomware diseñado para buscar y cifrar datos de usuarios.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}24){1} Morpheus                {2}Suite de Man-In-The-Middle que permite a los usuarios manipular tcp/udp.".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}25){1} FBTOOL                  {2}Kit de herramientas hacking de facebook".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print(
            "{0}26){1} Venom                   {2}Generador/compilador/controlador de shellcode (metasploit).".format(
                WHITE, YELLOW, DEFAULT
            )
        )
        print("{0} 0){1} Back".format(WHITE, YELLOW))
